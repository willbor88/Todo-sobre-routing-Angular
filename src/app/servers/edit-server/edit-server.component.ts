import { Component, OnInit } from '@angular/core';
import { ServersService } from '../servers.service';
import { ActivatedRoute,Router,Params} from "@angular/router";
import { canDeactivateComponent } from './canDeactivate-guard.service';
import { Observable } from 'rxjs/Observable';

@Component({
  selector: 'app-edit-server',
  templateUrl: './edit-server.component.html',
  styleUrls: ['./edit-server.component.css']
})
export class EditServerComponent implements OnInit,canDeactivateComponent {
  server: {id: number, name: string, status: string};
  serverName = '';
  serverStatus = '';
  permisos= false;
  cambiosrealizados= false

  constructor(private serversService: ServersService,
    private router:Router,
    private route:ActivatedRoute,
    
    ) { }

  ngOnInit() {
    
   console.log( this.route.snapshot.fragment)
   this.route.queryParams.subscribe(
    (queryParams:Params)=>{
      this.permisos=queryParams['permitirEditar']==='1'?true:false//recibe la propiedad para habilitar la edicion
    }
   )
   this.route.fragment.subscribe()
   const id= +this.route.snapshot.params['id']
   this.server = this.serversService.getServer(id);
    this.serverName = this.server.name;
    this.serverStatus = this.server.status;
  }

  onUpdateServer() {
    this.serversService.updateServer(this.server.id, {name: this.serverName, status: this.serverStatus});
    this.cambiosrealizados=true
    this.router.navigate(['../']),{relative:this.route}

}
CanDeactivate():Observable<boolean>|Promise<boolean>|boolean
{
if(!this.permisos){
  return true
  }
  if((this.serverName !==this.server.name||this.serverStatus !== this.server.status) && !this.cambiosrealizados)
  {
    return confirm('Deseas descartar los cambios?')
  }
  else{
    return true
  }
}

}
